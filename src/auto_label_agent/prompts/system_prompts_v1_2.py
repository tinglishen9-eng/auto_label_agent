...
PROMPT_DESCRIPTION = "默认相关性标注 prompt 包，包含需求理解、打分和质检三个阶段，并为每个阶段提供 few-shot 示例，同时引入独立账号意图判断信号。"
...
5. 输入里如果已经给出“账号意图判断”，要优先参考这个信号：
   - `single_account_intent` 表示更偏向单一账号搜索需求
   - `account_as_one_intent` 表示账号意图只是候选之一，仍要考虑普通语义意图
6. entities 和 qualifiers 尽量完整，不要臆造。"""
